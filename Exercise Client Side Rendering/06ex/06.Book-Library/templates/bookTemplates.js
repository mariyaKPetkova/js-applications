...
export let bookTemplate = (book, editHandler,delHandler) => html`
<tr class="book" data-id=${book._id}>
    <td>${book.title}</td>
    <td>${book.author}</td>
    <td>
        <button class="edit" @click=${editHandler}>Edit</button>
        <button class="delete" @click=${delHandler}>Delete</button>
    </td>
</tr>`;

export let allBooksTemplate = (books, editHandler,delHandler) => html`${books.map(b => bookTemplate(b, editHandler, delHandler))}`;

export let tableTemplate = (books, editHandler,delHandler) => html`
<table>
    <thead>
        <tr>
            <th>Title</th>
    ...
    <tbody id="books-container">
        ${allBooksTemplate(books, editHandler,delHandler)}
    </tbody>
</table>`;

export let bookLibaryTemplate = (books, forms, loadBooksHandler, editHandler,delHandler) => html`
   <button id="loadBooks" @click=${loadBooksHandler}>LOAD ALL BOOKS</button>
   ${tableTemplate(books, editHandler,delHandler)}
