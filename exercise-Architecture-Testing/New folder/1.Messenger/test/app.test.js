    ...
    describe('load messages', () => {
        it('should call server', async () => {
            await page.route('**/jsonstore/messenger', route => {
                route.fulfill(fakeResponse(testServerMessages))
            });
            ...
            expect(result).to.eql(testServerMessages);
        });
        ...
        it('should show data in text area', async () => {
            ...
            let text = Object.values(testServerMessages).map(v => `${v.author}: ${v.content}`).join('\n');
